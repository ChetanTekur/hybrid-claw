...
import { promptLocalModelsConfig } from "../commands/configure.local-models.js";
import { promptIdentityConfig } from "../commands/configure.identity.js";
...
export async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
    ...
    await promptIdentityConfig(workspaceDir, runtime);
    ...
    // Only prompt if hybridRouter is already configured or if this is a fresh setup
    const hasHybridRouter = nextConfig.agents?.defaults?.hybridRouter?.enabled;
    if (hasHybridRouter) {
        const configureLocal = await prompter.confirm({
            message: "Configure local Ollama models for hybrid routing?",
            initialValue: true,
        });
        if (configureLocal) {
            nextConfig = await promptLocalModelsConfig(nextConfig, runtime);
        }
    } else {
        // Offer to enable hybrid routing for new setups
        const enableLocal = await prompter.confirm({
            message: "Enable hybrid local/cloud routing? (uses free local models for simple tasks)",
            initialValue: false,
        });
        if (enableLocal) {
            nextConfig = await promptLocalModelsConfig(nextConfig, runtime);

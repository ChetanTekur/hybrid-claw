...
// Providers that indicate a cloud-hosted model (not local Ollama)
const CLOUD_PROVIDERS = new Set([
    "anthropic", "openai", "google", "openrouter", "xai", "mistral", "groq",
]);

/**
 * Check whether the most recent assistant message was generated by a
 * cloud provider.  OpenClaw's AssistantMessage carries `provider` and
 * `model` metadata on each message — we use that to detect cloud turns
 * so the router can maintain "session affinity" for multi-turn tool chains.
 */
export function wasLastAssistantCloud(messages) {
    for (let i = messages.length - 1; i >= 0; i--) {
        const m = messages[i];
        if (m?.role === "assistant") {
            // Direct provider field (set by OpenClaw agent loop)
            if (m.provider && CLOUD_PROVIDERS.has(m.provider)) return true;
            // Fallback: check model string "provider/modelId"
            if (m.model && typeof m.model === "string") {
                const providerPart = m.model.split("/")[0];
                if (CLOUD_PROVIDERS.has(providerPart)) return true;
            }
            return false; // Found an assistant message but it's local
        }
    }
    return false;
}
...
export function classifyComplexity(context, routingCfg) {
    ...
    // ---- Post-tool turn ----
    // If the previous assistant turn was cloud-generated, keep using cloud
    // so it can synthesise tool results with the full system prompt and
    // formatting capabilities.  Local post-tool turns stay local.
    if (isPostToolTurn(messages)) {
        if (wasLastAssistantCloud(messages)) {
            return { score: 1.0, reason: "cloud-continuation", tags: ["post-tool", "cloud-affinity"] };
        }
...
export function makeRoutingDecision(context, routerCfg, models, cfg, agentDir) {
    ...
    // Cloud continuation: previous assistant was cloud, keep affinity for synthesis
    if (reason === "cloud-continuation") {
        if (cloudAvailable) return pick("cloud", models.cloud, reason, score, tags);
        // Cloud unavailable — fall through to normal complexity routing
    }

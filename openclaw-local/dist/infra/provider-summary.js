...
import { listDiscordAccountIds, resolveDiscordAccount, } from "../discord/accounts.js";
import { listIMessageAccountIds, resolveIMessageAccount, } from "../imessage/accounts.js";
...
export async function buildProviderSummary(cfg, options) {
    ...
    if (!webEnabled) {
        ...
        lines.push(webLinked
            ? tint(`WhatsApp: linked${e164 ? ` ${e164}` : ""}${authAge}`, theme.success)
        ...
        if (webLinked) {
            for (const accountId of listWhatsAppAccountIds(effective)) {
                ...
                if (account.selfChatMode)
                    details.push("self-chat");
                const dmPolicy = account.dmPolicy ?? effective.whatsapp?.dmPolicy ?? "pairing";
    ...
    if (!telegramEnabled) {
        ...
        const accounts = listTelegramAccountIds(effective).map((accountId) => resolveTelegramAccount({ cfg: effective, accountId }));
        const configuredAccounts = accounts.filter((account) => Boolean(account.token?.trim()));
    ...
    if (!discordEnabled) {
        ...
        const accounts = listDiscordAccountIds(effective).map((accountId) => resolveDiscordAccount({ cfg: effective, accountId }));
        const configuredAccounts = accounts.filter((account) => Boolean(account.token?.trim()));
        const discordConfigured = configuredAccounts.length > 0;
        lines.push(discordConfigured
    ...
    if (!slackEnabled) {
        ...
        const accounts = listSlackAccountIds(effective).map((accountId) => resolveSlackAccount({ cfg: effective, accountId }));
        const configuredAccounts = accounts.filter((account) => Boolean(account.botToken?.trim()) && Boolean(account.appToken?.trim()));
        const slackConfigured = configuredAccounts.length > 0;
        lines.push(slackConfigured
    ...
    if (!signalEnabled) {
        ...
        const accounts = listSignalAccountIds(effective).map((accountId) => resolveSignalAccount({ cfg: effective, accountId }));
    ...
    if (!imessageEnabled) {
        ...
        const accounts = listIMessageAccountIds(effective).map((accountId) => resolveIMessageAccount({ cfg: effective, accountId }));
        const configuredAccounts = accounts.filter((account) => Boolean(account.config.cliPath ||

...
export const providersHandlers = {
    "providers.status": async ({ params, respond, context }) => {
        ...
        const telegramAccounts = await Promise.all(listTelegramAccountIds(cfg).map(async (accountId) => {
            const account = resolveTelegramAccount({ cfg, accountId });
            const rt = runtime.telegramAccounts?.[account.accountId] ??
            ...
            if (probe && configured && account.enabled) {
                telegramProbe = await probeTelegram(account.token, timeoutMs, account.config.proxy);
                lastProbeAt = Date.now();
                const botId = telegramProbe.ok && telegramProbe.bot?.id != null
        ...
        const defaultTelegramAccount = telegramAccounts.find((account) => account.accountId === defaultTelegramAccountId) ?? telegramAccounts[0];
        const discordAccounts = await Promise.all(listDiscordAccountIds(cfg).map(async (accountId) => {
            const account = resolveDiscordAccount({ cfg, accountId });
            const rt = runtime.discordAccounts?.[account.accountId] ??
            ...
            let discordProbe;
            let lastProbeAt = null;
            const { channelIds: auditChannelIds, unresolvedChannels } = collectDiscordAuditChannelIds({ cfg, accountId: account.accountId });
            let audit;
            if (probe && configured && account.enabled) {
        ...
        const defaultDiscordAccount = discordAccounts.find((account) => account.accountId === defaultDiscordAccountId) ?? discordAccounts[0];
        const slackAccounts = await Promise.all(listSlackAccountIds(cfg).map(async (accountId) => {
            const account = resolveSlackAccount({ cfg, accountId });
            const rt = runtime.slackAccounts?.[account.accountId] ??
        ...
        const defaultSlackAccount = slackAccounts.find((account) => account.accountId === defaultSlackAccountId) ?? slackAccounts[0];
        const signalAccounts = await Promise.all(listSignalAccountIds(cfg).map(async (accountId) => {
            const account = resolveSignalAccount({ cfg, accountId });
            const rt = runtime.signalAccounts?.[account.accountId] ??
        ...
        const defaultSignalAccount = signalAccounts.find((account) => account.accountId === defaultSignalAccountId) ?? signalAccounts[0];
        ...
        const imessageAccounts = listIMessageAccountIds(cfg).map((accountId) => {
            ...
            const rt = runtime.imessageAccounts?.[account.accountId] ??
        ...
        const defaultIMessageAccount = imessageAccounts.find((account) => account.accountId === defaultIMessageAccountId) ?? imessageAccounts[0];
        ...
        const defaultWhatsAppAccount = enabledWhatsAppAccounts.find((account) => account.accountId === defaultWhatsAppAccountId) ?? enabledWhatsAppAccounts[0];
        ...
        const whatsappAccounts = await Promise.all(enabledWhatsAppAccounts.map(async (account) => {
            const rt = runtime.whatsappAccounts?.[account.accountId] ??

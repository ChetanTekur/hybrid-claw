...
import { listEnabledWhatsAppAccounts, resolveDefaultWhatsAppAccountId, } from "../web/accounts.js";
import { readWebSelfId } from "../web/session.js";
import { formatError } from "./server-utils.js";
export function createProviderManager(opts) {
    const { loadConfig, logWhatsApp, logTelegram, logDiscord, logSlack, logSignal, logIMessage, whatsappRuntimeEnv, telegramRuntimeEnv, discordRuntimeEnv, slackRuntimeEnv, signalRuntimeEnv, imessageRuntimeEnv, } = opts;
    const whatsappAborts = new Map();
    const telegramAborts = new Map();
    const discordAborts = new Map();
    ...
    const signalTasks = new Map();
    const imessageTasks = new Map();
    ...
    const telegramRuntimes = new Map();
    const discordRuntimes = new Map();
    const slackRuntimes = new Map();
    const signalRuntimes = new Map();
    const imessageRuntimes = new Map();
    ...
    const defaultSlackStatus = () => ({
        running: false,
        lastStartAt: null,
        lastStopAt: null,
        lastError: null,
    });
    const defaultSignalStatus = () => ({
        running: false,
        lastStartAt: null,
        lastStopAt: null,
        lastError: null,
    ...
    const updateWhatsAppStatus = (accountId, next) => {
        whatsappRuntimes.set(accountId, next);
    };
    const startWhatsAppProvider = async (accountId) => {
        ...
        if (cfg.web?.enabled === false) {
            for (const account of targets) {
                const current = whatsappRuntimes.get(account.accountId) ?? defaultWhatsAppStatus();
        ...
        await Promise.all(targets.map(async (account) => {
            if (whatsappTasks.has(account.accountId))
                return;
            const current = whatsappRuntimes.get(account.accountId) ?? defaultWhatsAppStatus();
            if (!(await webAuthExists(account.authDir))) {
                ...
                logWhatsApp.info(`[${account.accountId}] skipping provider start (no linked session)`);
                return;
            }
            const { e164, jid } = readWebSelfId(account.authDir);
            const identity = e164 ? e164 : jid ? `jid ${jid}` : "unknown";
            logWhatsApp.info(`[${account.accountId}] starting provider (${identity})`);
            ...
            const task = monitorWebProvider(shouldLogVerbose(), undefined, true, undefined, whatsappRuntimeEnv, abort.signal, {
                ...
                logWhatsApp.error(`[${account.accountId}] provider exited: ${formatError(err)}`);
            })
                .finally(() => {
    ...
    const stopWhatsAppProvider = async (accountId) => {
        const ids = accountId
            ? [accountId]
            : Array.from(new Set([...whatsappAborts.keys(), ...whatsappTasks.keys()]));
    ...
    const startTelegramProvider = async (accountId) => {
        const cfg = loadConfig();
        ...
        await Promise.all(accountIds.map(async (id) => {
            ...
            if (!account.enabled) {
                const latest = telegramRuntimes.get(account.accountId) ?? defaultTelegramStatus();
            ...
            if (telegramTasks.has(account.accountId))
                return;
            ...
            if (!token) {
                ...
                if (shouldLogVerbose()) {
                    logTelegram.debug(`[${account.accountId}] telegram provider not configured (no TELEGRAM_BOT_TOKEN)`);
                }
                return;
            }
            ...
            try {
                ...
                if (shouldLogVerbose()) {
                    logTelegram.debug(`[${account.accountId}] bot probe failed: ${String(err)}`);
                }
            }
            logTelegram.info(`[${account.accountId}] starting provider${telegramBotLabel}`);
            const abort = new AbortController();
            ...
            const task = monitorTelegramProvider({
                ...
                .catch((err) => {
                const current = telegramRuntimes.get(account.accountId) ??
                    defaultTelegramStatus();
                telegramRuntimes.set(account.accountId, {
                    ...current,
                    lastError: formatError(err),
                });
                logTelegram.error(`[${account.accountId}] provider exited: ${formatError(err)}`);
            })
                .finally(() => {
    ...
    const stopTelegramProvider = async (accountId) => {
        const ids = accountId
            ? [accountId]
            : Array.from(new Set([...telegramAborts.keys(), ...telegramTasks.keys()]));
        await Promise.all(ids.map(async (id) => {
    ...
    const startDiscordProvider = async (accountId) => {
        const cfg = loadConfig();
        ...
        await Promise.all(accountIds.map(async (id) => {
            ...
            if (!account.enabled) {
                const latest = discordRuntimes.get(account.accountId) ?? defaultDiscordStatus();
            ...
            if (discordTasks.has(account.accountId))
                return;
            const token = account.token.trim();
            if (!token) {
                const latest = discordRuntimes.get(account.accountId) ?? defaultDiscordStatus();
                ...
                if (shouldLogVerbose()) {
                    logDiscord.debug(`[${account.accountId}] discord provider not configured (no DISCORD_BOT_TOKEN)`);
                }
                return;
            }
            let discordBotLabel = "";
            try {
                ...
                if (messageContent === "disabled") {
                    logDiscord.warn(`[${account.accountId}] Discord Message Content Intent is disabled; bot may not respond to channel messages. Enable it in Discord Dev Portal (Bot → Privileged Gateway Intents) or require mentions.`);
                }
                else if (messageContent === "limited") {
                    logDiscord.info(`[${account.accountId}] Discord Message Content Intent is limited; bots under 100 servers can use it without verification.`);
                }
            }
            catch (err) {
                if (shouldLogVerbose()) {
                    logDiscord.debug(`[${account.accountId}] bot probe failed: ${String(err)}`);
                }
            }
            logDiscord.info(`[${account.accountId}] starting provider${discordBotLabel}`);
            const abort = new AbortController();
            ...
            const task = monitorDiscordProvider({
                ...
                .catch((err) => {
                const current = discordRuntimes.get(account.accountId) ?? defaultDiscordStatus();
                discordRuntimes.set(account.accountId, {
                    ...current,
                    lastError: formatError(err),
                });
                logDiscord.error(`[${account.accountId}] provider exited: ${formatError(err)}`);
            })
                .finally(() => {
    ...
    const stopDiscordProvider = async (accountId) => {
        const ids = accountId
            ? [accountId]
    ...
    const startSlackProvider = async (accountId) => {
        const cfg = loadConfig();
        ...
        await Promise.all(accountIds.map(async (id) => {
            ...
            if (!account.enabled) {
                const latest = slackRuntimes.get(account.accountId) ?? defaultSlackStatus();
            ...
            if (slackTasks.has(account.accountId))
                return;
            ...
            if (!botToken || !appToken) {
                ...
                if (shouldLogVerbose()) {
                    logSlack.debug(`[${account.accountId}] slack provider not configured (missing SLACK_BOT_TOKEN/SLACK_APP_TOKEN)`);
                }
                return;
            }
            ...
            const task = monitorSlackProvider({
                ...
                .catch((err) => {
                const current = slackRuntimes.get(account.accountId) ?? defaultSlackStatus();
                slackRuntimes.set(account.accountId, {
                    ...current,
                    lastError: formatError(err),
                });
                logSlack.error(`[${account.accountId}] provider exited: ${formatError(err)}`);
            })
                .finally(() => {
    ...
    const stopSlackProvider = async (accountId) => {
        const ids = accountId
            ? [accountId]
    ...
    const startSignalProvider = async (accountId) => {
        const cfg = loadConfig();
        ...
        await Promise.all(accountIds.map(async (id) => {
            ...
            if (!account.enabled) {
                const latest = signalRuntimes.get(account.accountId) ?? defaultSignalStatus();
            ...
            if (!account.configured) {
                ...
                if (shouldLogVerbose()) {
                    logSignal.debug(`[${account.accountId}] signal provider not configured (missing signal config)`);
                }
                return;
            }
            if (signalTasks.has(account.accountId))
                return;
            logSignal.info(`[${account.accountId}] starting provider (${account.baseUrl})`);
            const abort = new AbortController();
            ...
            const task = monitorSignalProvider({
                ...
                .catch((err) => {
                const current = signalRuntimes.get(account.accountId) ?? defaultSignalStatus();
                signalRuntimes.set(account.accountId, {
                    ...current,
                    lastError: formatError(err),
                });
                logSignal.error(`[${account.accountId}] provider exited: ${formatError(err)}`);
            })
                .finally(() => {
    ...
    const stopSignalProvider = async (accountId) => {
        const ids = accountId
            ? [accountId]
    ...
    const startIMessageProvider = async (accountId) => {
        const cfg = loadConfig();
        ...
        await Promise.all(accountIds.map(async (id) => {
            ...
            if (!account.enabled) {
                const latest = imessageRuntimes.get(account.accountId) ?? defaultIMessageStatus();
                ...
                if (shouldLogVerbose()) {
                    logIMessage.debug(`[${account.accountId}] imessage provider disabled (imessage.enabled=false)`);
                }
                return;
            }
            if (imessageTasks.has(account.accountId))
                return;
            const cliPath = account.config.cliPath?.trim() || "imsg";
            const dbPath = account.config.dbPath?.trim();
            logIMessage.info(`[${account.accountId}] starting provider (${cliPath}${dbPath ? ` db=${dbPath}` : ""})`);
            const abort = new AbortController();
            ...
            const task = monitorIMessageProvider({
                ...
                .catch((err) => {
                const current = imessageRuntimes.get(account.accountId) ??
                    defaultIMessageStatus();
                imessageRuntimes.set(account.accountId, {
                    ...current,
                    lastError: formatError(err),
                });
                logIMessage.error(`[${account.accountId}] provider exited: ${formatError(err)}`);
            })
                .finally(() => {
    ...
    const stopIMessageProvider = async (accountId) => {
        const ids = accountId
            ? [accountId]
            : Array.from(new Set([...imessageAborts.keys(), ...imessageTasks.keys()]));
        await Promise.all(ids.map(async (id) => {
    ...
    const markWhatsAppLoggedOut = (cleared, accountId) => {
        const cfg = loadConfig();
    ...
    const getRuntimeSnapshot = () => {
        ...
        const whatsapp = whatsappRuntimes.get(defaultWhatsAppId) ?? defaultWhatsAppStatus();
        const whatsappAccounts = Object.fromEntries(Array.from(whatsappRuntimes.entries()).map(([id, status]) => [
            id,
            { ...status },
        ]));
        const telegramAccounts = Object.fromEntries(listTelegramAccountIds(cfg).map((id) => {
            const account = resolveTelegramAccount({ cfg, accountId: id });
            const current = telegramRuntimes.get(account.accountId) ?? defaultTelegramStatus();
            const status = {
                ...current,
                mode: current.mode ?? (account.config.webhookUrl ? "webhook" : "polling"),
            };
            if (!status.running) {
                if (!account.enabled) {
        ...
        const telegram = telegramAccounts[telegramDefaultId] ?? defaultTelegramStatus();
        const discordAccounts = Object.fromEntries(listDiscordAccountIds(cfg).map((id) => {
            const account = resolveDiscordAccount({ cfg, accountId: id });
            const current = discordRuntimes.get(account.accountId) ?? defaultDiscordStatus();
        ...
        const slackAccounts = Object.fromEntries(listSlackAccountIds(cfg).map((id) => {
            const account = resolveSlackAccount({ cfg, accountId: id });
            const current = slackRuntimes.get(account.accountId) ?? defaultSlackStatus();
            const status = { ...current };
            if (!status.running) {
        ...
        const signalAccounts = Object.fromEntries(listSignalAccountIds(cfg).map((id) => {
            const account = resolveSignalAccount({ cfg, accountId: id });
            const current = signalRuntimes.get(account.accountId) ?? defaultSignalStatus();
            const status = {
                ...current,
        ...
        const imessageAccounts = Object.fromEntries(listIMessageAccountIds(cfg).map((id) => {
            const account = resolveIMessageAccount({ cfg, accountId: id });
            const current = imessageRuntimes.get(account.accountId) ?? defaultIMessageStatus();
            const cliPath = account.config.cliPath?.trim() || "imsg";
        ...
        const imessage = imessageAccounts[imessageDefaultId] ?? defaultIMessageStatus();
        return {
            whatsapp: { ...whatsapp },
            whatsappAccounts,
            telegram,

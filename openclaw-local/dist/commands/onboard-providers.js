...
import { formatProviderPrimerLine, formatProviderSelectionLine, listChatProviders, } from "../providers/registry.js";
...
export function setWhatsAppDmPolicy(cfg, dmPolicy) {
    return mergeWhatsAppConfig(cfg, { dmPolicy });
}
export function setWhatsAppAllowFrom(cfg, allowFrom) {
...
export function setWhatsAppSelfChatMode(cfg, selfChatMode) {
...
async function maybeConfigureDmPolicies(params) {
    ...
    const supportsDmPolicy = selection.some((p) => ["telegram", "discord", "slack", "signal", "imessage"].includes(p));
    if (!supportsDmPolicy)
        return params.cfg;
    ...
    if (selection.includes("telegram")) {
        ...
        if (policy !== current)
            cfg = setTelegramDmPolicy(cfg, policy);
    }
    if (selection.includes("discord")) {
        ...
        if (policy !== current)
            cfg = setDiscordDmPolicy(cfg, policy);
    }
    ...
    if (selection.includes("signal")) {
        ...
        if (policy !== current)
            cfg = setSignalDmPolicy(cfg, policy);
    }
    if (selection.includes("imessage")) {
        ...
        if (policy !== current)
            cfg = setIMessageDmPolicy(cfg, policy);
    }
    return cfg;
}
...
export async function setupProviders(cfg, runtime, prompter, options) {
    ...
    let whatsappAccountId = options?.whatsappAccountId?.trim() || resolveDefaultWhatsAppAccountId(cfg);
    ...
    const telegramConfigured = listTelegramAccountIds(cfg).some((accountId) => Boolean(resolveTelegramAccount({ cfg, accountId }).token));
    const discordConfigured = listDiscordAccountIds(cfg).some((accountId) => Boolean(resolveDiscordAccount({ cfg, accountId }).token));
    const slackConfigured = listSlackAccountIds(cfg).some((accountId) => {
        const account = resolveSlackAccount({ cfg, accountId });
        return Boolean(account.botToken && account.appToken);
    });
    const signalConfigured = listSignalAccountIds(cfg).some((accountId) => resolveSignalAccount({ cfg, accountId }).configured);
    ...
    const waAccountLabel = whatsappAccountId === DEFAULT_ACCOUNT_ID ? "default" : whatsappAccountId;
    await prompter.note([
                ...
                whatsapp: {
                    ...
                    accounts: {
                        ...
                        [whatsappAccountId]: {
                            ...
                            enabled: next.whatsapp?.accounts?.[whatsappAccountId]?.enabled ?? true,
                        },
                    },
                },
            };

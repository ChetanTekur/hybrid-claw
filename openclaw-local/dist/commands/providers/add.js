...
export async function providersAddCommand(opts, runtime = defaultRuntime, params) {
    ...
    if (useWizard) {
        ...
        if (wantsNames) {
            for (const provider of selection) {
                ...
                const existingName = provider === "whatsapp"
    ...
    if (provider === "telegram") {
        if (useEnv && accountId !== DEFAULT_ACCOUNT_ID) {
            runtime.error("TELEGRAM_BOT_TOKEN can only be used for the default account.");
            runtime.exit(1);
            return;
        }
        if (!useEnv && !opts.token && !opts.tokenFile) {
            runtime.error("Telegram requires --token or --token-file (or --use-env).");
            runtime.exit(1);
            return;
        }
    }
    if (provider === "discord") {
        if (useEnv && accountId !== DEFAULT_ACCOUNT_ID) {
            runtime.error("DISCORD_BOT_TOKEN can only be used for the default account.");
            runtime.exit(1);
            return;
        }
    ...
    if (provider === "slack") {
        if (useEnv && accountId !== DEFAULT_ACCOUNT_ID) {
            runtime.error("Slack env tokens can only be used for the default account.");
            runtime.exit(1);
            return;
        }
        if (!useEnv && (!opts.botToken || !opts.appToken)) {
            runtime.error("Slack requires --bot-token and --app-token (or --use-env).");
            runtime.exit(1);
            return;
        }
    }
    if (provider === "signal") {
        if (!opts.signalNumber &&
            !opts.httpUrl &&
            !opts.httpHost &&
            !opts.httpPort &&
            !opts.cliPath) {
            runtime.error("Signal requires --signal-number or --http-url/--http-host/--http-port/--cli-path.");
            runtime.exit(1);
            return;
        }
    }

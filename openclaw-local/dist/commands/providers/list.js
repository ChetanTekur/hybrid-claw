...
import { listWhatsAppAccountIds, resolveWhatsAppAuthDir, } from "../../web/accounts.js";
...
export async function providersListCommand(opts, runtime = defaultRuntime) {
    ...
    const lineBuilders = {
        telegram: async (accountId) => {
            ...
            return `- ${label}: ${formatConfigured(Boolean(account.token))}, ${formatTokenSource(account.tokenSource)}, ${formatEnabled(account.enabled)}`;
        },
        whatsapp: async (accountId) => {
            ...
            return `- ${label}: ${formatLinked(linked)}, ${formatEnabled(cfg.whatsapp?.accounts?.[accountId]?.enabled ??
                cfg.web?.enabled ??
                true)}`;
        },
        ...
        slack: async (accountId) => {
            ...
            return `- ${label}: ${formatConfigured(configured)}, ${formatSource("bot", account.botTokenSource)}, ${formatSource("app", account.appTokenSource)}, ${formatEnabled(account.enabled)}`;
        },
        signal: async (accountId) => {
            ...
            return `- ${label}: ${formatConfigured(account.configured)}, base=${theme.muted(account.baseUrl)}, ${formatEnabled(account.enabled)}`;
        },

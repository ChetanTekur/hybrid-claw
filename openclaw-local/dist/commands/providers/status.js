...
export function formatGatewayProvidersStatusLines(payload) {
    ...
    lines.push(`Tip: ${formatDocsLink("/cli#status", "status --deep")} runs local probes without a gateway.`);
    return lines;
}
async function formatConfigProvidersStatusLines(cfg, meta) {
    ...
    const accounts = {
        ...
        imessage: listIMessageAccountIds(cfg).map((accountId) => {
            ...
            const imsgConfigured = Boolean(account.config.cliPath ||
    ...
    try {
        ...
        lines.push(`WhatsApp: ${webLinked ? "linked" : "not linked"}${e164 ? ` ${e164}` : ""}${webLinked ? authAge : ""}`);
    }
    catch {
        // ignore
    }

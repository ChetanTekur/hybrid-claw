...
function providerHasAccounts(cfg, provider) {
    if (provider === "whatsapp")
        return true;
    const base = cfg[provider];
    return Boolean(base?.accounts && Object.keys(base.accounts).length > 0);
}
function shouldStoreNameInAccounts(cfg, provider, accountId) {
    if (provider === "whatsapp")
        return true;
...
function migrateBaseNameToDefaultAccount(cfg, provider) {
    if (provider === "whatsapp")
        return cfg;
    const base = cfg[provider];
...
export function applyAccountName(params) {
    ...
    if (!useAccounts && accountId === DEFAULT_ACCOUNT_ID) {
        const baseConfig = params.cfg[key];
        const safeBase = typeof baseConfig === "object" && baseConfig
            ? baseConfig
            : {};
        return {
            ...params.cfg,
